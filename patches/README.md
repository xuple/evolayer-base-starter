...
2. Re-run `php artisan evolayer:ai:stream-check gemini` and `openai` against the unpatched vendor copy. Also re-check Anthropic once its structured-streaming path emits `TextDelta` events. If runtime-approved providers pass without the patch, delete this patch and the `extra.patches` entry in `composer.json`.
