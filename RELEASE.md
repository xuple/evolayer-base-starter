...
is `xuple/evolayer-base-starter v0.1.17` consuming `xuple/evolayer-base` (exact-pinned while `0.x`, at `0.1.8`). GitHub is the public publication source; the
...
Current starter release: **0.1.17** (pre-1.0 developer preview). Future fixes use
...
```jsonc
"require": { "xuple/evolayer-base": "0.1.8" }
```
...
and published on Packagist**. The published starter `v0.1.17` consumes the base package exact-pinned while `0.x` (at `0.1.8`)
